## SHFM ##
## cstat and auc together, mortality, 1yr horizon
...
figfile <- "Fig2g_S_1yr_ACMapprox_disc_cstatauc.jpg"
...
rma.res <- fn_ma_cstat(dat)
resS_disc_ACM_1yr_auc <- rma.res[[2]]
res.auc <- rma.res[[3]]
...
rma.res <- fn_ma_cstat(dat)
resS_disc_ACM_1yr_cstat <- rma.res[[2]]
res.cstat <- rma.res[[3]]
...
forest(y$theta, ci.lb=y$theta.ciub, ci.ub=y$theta.cilb,
       ...
       rows=c(40:16,11:3),      # CHANGE studies within groups
       ylim=(c(-5,nrow(y)+10)),
       main=titplot
)
...
text(-0.3, c(41,12), pos=4, c("Discrimination assessed using AUC",
...
addpoly(x = resS_disc_ACM_1yr_auc$est, ci.lb=resS_disc_ACM_1yr_auc$ci.lb, ci.ub=resS_disc_ACM_1yr_auc$ci.ub,
        row= 14.5, mlab=mlabfun("RE Model for Subgroup", res.auc))
addpoly(x = resS_disc_ACM_1yr_cstat$est, ci.lb=resS_disc_ACM_1yr_cstat$ci.lb, ci.ub=resS_disc_ACM_1yr_cstat$ci.ub,
