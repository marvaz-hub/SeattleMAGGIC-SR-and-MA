...
figfile <- "Fig3e_S_1yr_ACMapprox_cal_rob.jpg"
...
#   29      16       2       0 
#exclude 'Unclear'
df_ma <- df_ma[which(df_ma$OS1!="Unclear"),]
...
table(dat$horizon, useNA = "always") #26
...
table(dat2$OS1, useNA = "always") #7


# dat3 <- dat[which(dat$OS1=="Unclear"),]
# 
# table(dat3$OS1, useNA = "always") #2 out
...
rma.res <- fn_ma_oe(dat)
resS_cal_ACM_1yr_hrob <- rma.res[[2]]
res.hrob <- rma.res[[3]]
...
rma.res <- fn_ma_oe(dat)
resS_cal_ACM_1yr_lrob <- rma.res[[2]]
res.lrob <- rma.res[[3]]
...
# 
# rma.res <- fn_ma_oe(dat)
# resS_cal_ACM_1yr_urob <- rma.res[[2]]
# res.urob <- rma.res[[3]]

resS_cal_ACM_1yr_urob <- c(rep("-",5), 2, "-")
...
metafor::forest(y$theta, ci.lb=y$theta.cilb,ci.ub=y$theta.ciub,
                ...
                rows=c(32:14,9:3), # CHANGE studies within groups
                ylim=(c(-5,nrow(y)+10)),
...
text(-0.3, c(33,10), pos=4, c("High RoB Participants",
...
addpoly(x = resS_cal_ACM_1yr_26$est, ci.lb=resS_cal_ACM_1yr_26$ci.lb, ci.ub=resS_cal_ACM_1yr_26$ci.ub,
        row=-2, mlab=mlabfun("Overall", res.26))

### add summary polygons for the three subgroups (CHANGE res variables and rows for the polygon)
addpoly(x = resS_cal_ACM_1yr_hrob$est, ci.lb=resS_cal_ACM_1yr_hrob$ci.lb, ci.ub=resS_cal_ACM_1yr_hrob$ci.ub,
        row=12.5, mlab=mlabfun("RE Model for Subgroup", res.hrob))
addpoly(x = resS_cal_ACM_1yr_lrob$est, ci.lb=resS_cal_ACM_1yr_lrob$ci.lb, ci.ub=resS_cal_ACM_1yr_lrob$ci.ub,
        row= 1.5, mlab=mlabfun("RE Model for Subgroup", res.lrob))
# addpoly(x = resS_cal_ACM_1yr_urob$est, ci.lb=resS_cal_ACM_1yr_urob$ci.lb, ci.ub=resS_cal_ACM_1yr_urob$ci.ub,
