...
figfile <- "Fig2c_M_1yr_ACMapprox_disc_cstatauc.jpg"
...
table(dat1$S4, useNA = "always") #9


dat2 <- dat[which(dat$S4=="c-stat"),] 

table(dat2$S4, useNA = "always") #14
...
rma.res <- fn_ma_cstat(dat)
resM_disc_ACM_1yr_auc <- rma.res[[2]]
res.auc <- rma.res[[3]]
...
rma.res <- fn_ma_cstat(dat)
resM_disc_ACM_1yr_cstat <- rma.res[[2]]
res.cstat <- rma.res[[3]]
...
forest(y$theta, ci.lb=y$theta.ciub, ci.ub=y$theta.cilb,
       ...
       rows=c(29:21,16:3), # CHANGE studies within groups
       ylim=(c(-5,nrow(y)+10)), # CHANGE if number of groups !=3
       main=titplot
)
...
text(-0.3, c(30,17), pos=4, c("Discrimination assessed using AUC",
...
addpoly(x = resM_disc_ACM_1yr_auc$est, ci.lb=resM_disc_ACM_1yr_auc$ci.lb, ci.ub=resM_disc_ACM_1yr_auc$ci.ub,
        row= 19.5, mlab=mlabfun("RE Model for Subgroup", res.auc))
addpoly(x = resM_disc_ACM_1yr_cstat$est, ci.lb=resM_disc_ACM_1yr_cstat$ci.lb, ci.ub=resM_disc_ACM_1yr_cstat$ci.ub,
...
par(cex=1.1, font=2)
text(c(-0.3, pos_event, pos_N, pos_CI-.33), nrow(y)+9 , pos = c(4,2,2,2) , #CHANGE nrow(y)+13 if no. sugbroups !=3
