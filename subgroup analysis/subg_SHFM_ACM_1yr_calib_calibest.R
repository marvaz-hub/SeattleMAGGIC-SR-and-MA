...
figfile <- "Fig3g_S_1yr_ACMapprox_cal_calibest.jpg"
...
rma.res <- fn_ma_oe(dat)
resS_cal_ACM_1yr_oeestfrominterc <- rma.res[[2]]
...
rma.res <- fn_ma_oe(dat)
resS_cal_ACM_1yr_oerep <- rma.res[[2]]
res.oerep <- rma.res[[3]]
...
metafor::forest(y$theta, ci.lb=y$theta.cilb,ci.ub=y$theta.ciub,
                ...
                rows=c(34:31,26:3), # CHANGE studies within groups
                ylim=(c(-5,nrow(y)+10)),
...
text(-0.3, c(35,27), pos=4,  c("Estimates based on intercept of calibration curve",
...
addpoly(x = resS_cal_ACM_1yr_oeestfrominterc$est, ci.lb=resS_cal_ACM_1yr_oeestfrominterc$ci.lb, ci.ub=resS_cal_ACM_1yr_oeestfrominterc$ci.ub,
        row=29.5, mlab=mlabfun("RE Model for Subgroup", res.hrob))
addpoly(x = resS_cal_ACM_1yr_oerep$est, ci.lb=resS_cal_ACM_1yr_oerep$ci.lb, ci.ub=resS_cal_ACM_1yr_oerep$ci.ub,
