## SHFM ##
## cstat and auc together, mortality, 60m horizons
...
# figfile <- "Fig7g_S_5yr_ACMapprox_disc_cstatauc.jpg"
...
table(dat$horizon, useNA = "always") #33
...
table(dat1$S4, useNA = "always") #24


# dat2 <- dat[which(dat$S4=="c-stat"),] 
# 
# table(dat2$S4, useNA = "always") #9
...
rma.res <- fn_ma_cstat(dat)
resS_disc_ACM_5yr_auc <- rma.res[[2]]
res.auc <- rma.res[[3]]
...
# 
# rma.res <- fn_ma_cstat(dat)
# resS_disc_ACM_5yr_cstat <- rma.res[[2]]
# res.cstat <- rma.res[[3]]
#
resS_disc_ACM_5yr_cstat <- c(rep("-",5), 2, "-") 
...
# addpoly(x = resS_disc_ACM_5yr_auc$est, ci.lb=resS_disc_ACM_5yr_auc$ci.lb, ci.ub=resS_disc_ACM_5yr_auc$ci.ub,
#         row= 14.5, mlab=mlabfun("RE Model for Subgroup", res.auc))
# addpoly(x = resS_disc_ACM_5yr_cstat$est, ci.lb=resS_disc_ACM_5yr_cstat$ci.lb, ci.ub=resS_disc_ACM_5yr_cstat$ci.ub,
