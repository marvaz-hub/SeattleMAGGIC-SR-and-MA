...
nstudies <- nrow(oe_all_log) #38

## univariate MR
resS_cal_ACM_univmr_year <- rma(yi=theta, sei=theta.se, mods = ~ year, data=oe_all_log, method="REML", test="knha")
resS_cal_ACM_univmr_outc <- rma(yi=theta, sei=theta.se, mods = ~ noofoutcomes, data=oe_all_log, method="REML", test="knha")
resS_cal_ACM_univmr_hor <- rma(yi=theta, sei=theta.se, mods = ~ horizon, data=oe_all_log, method="REML", test="knha")
resS_cal_ACM_univmr_sdage <- rma(yi=theta, sei=theta.se, mods = ~ sd1, data=oe_all_log, method="REML", test="knha")

## multivariable MR
# resS_cal_ACM_mr <- rma(yi=theta, sei=theta.se, mods = ~ year + noofoutcomes + horizon, data=oe_all_log, method="REML", test="knha")
...
table(dat$year)
# 2006 2009 2011 2012 2013 2014 2015 2016 2017 2018 2019 2020 2021 2022 2023 
# 13    3    3    6    2    3    1    3    1    2    2    1    4    1    2 

dat$noofoutcomes<- round(dat$o.est,0)
summary(dat$noofoutcomes)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 1.0    34.5    82.0   250.3   328.5  1727.0  
...
# remove NA SE(OE)
oe_all_log <- oe_all_log[which(!is.na(oe_all_log$theta.se)),]  
nstudies <- nrow(oe_all_log) #47

## univariate MR
resS_cal_ACMcomp_univmr_year <- rma(yi=theta, sei=theta.se, mods = ~ year, data=oe_all_log, method="REML", test="knha")
resS_cal_ACMcomp_univmr_outc <- rma(yi=theta, sei=theta.se, mods = ~ noofoutcomes, data=oe_all_log, method="REML", test="knha")
resS_cal_ACMcomp_univmr_hor <- rma(yi=theta, sei=theta.se, mods = ~ horizon, data=oe_all_log, method="REML", test="knha")
resS_cal_ACMcomp_univmr_sdage <- rma(yi=theta, sei=theta.se, mods = ~ sd1, data=oe_all_log, method="REML", test="knha")

## multivariable MR
# resS_cal_ACMcomp_mr <- rma(yi=theta, sei=theta.se, mods = ~ year + noofoutcomes + horizon, data=oe_all_log, method="REML", test="knha")
...
## univariate MR
resS_cal_ACM_nonestHor_univmr_year <- rma(yi=theta, sei=theta.se, mods = ~ year, data=oe_all_log, method="REML", test="knha")
resS_cal_ACM_nonestHor_univmr_outc <- rma(yi=theta, sei=theta.se, mods = ~ noofoutcomes, data=oe_all_log, method="REML", test="knha")
resS_cal_ACM_nonestHor_univmr_hor <- rma(yi=theta, sei=theta.se, mods = ~ horizon, data=oe_all_log, method="REML", test="knha")
resS_cal_ACM_nonestHor_univmr_sdage <- rma(yi=theta, sei=theta.se, mods = ~ sd1, data=oe_all_log, method="REML", test="knha")

## multivariable MR
# resS_cal_ACM_nonestHor_mr <- rma(yi=theta, sei=theta.se, mods = ~ year + noofoutcomes + horizon, data=oe_all_log, method="REML", test="knha")
